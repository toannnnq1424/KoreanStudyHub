...
document.addEventListener('DOMContentLoaded', () => {
  ...
  document.querySelectorAll('[data-assignment-status-shortcut]').forEach((shortcut) => {
    shortcut.addEventListener('click', () => {
      if (status) {
        status.value = shortcut.dataset.assignmentStatusShortcut;
  ...
  const syncStatusShortcuts = () => {
    const state = status?.value || 'ALL';
    document.querySelectorAll('[data-assignment-status-shortcut]').forEach((item) => {
      item.classList.toggle('is-active', item.dataset.assignmentStatusShortcut === state);
    });
  };
  const clearDetail = () => {
    rows.forEach((item) => {
  ...
  const setDetail = (row) => {
    if (!detail || !row || row.hidden) return;
    detail.hidden = false;
    rows.forEach((item) => {
  ...
  const initial = rows.find((row) => row.dataset.assignmentSelected === 'true') || rows[0];
