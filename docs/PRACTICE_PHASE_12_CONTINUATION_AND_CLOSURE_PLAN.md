...
Trạng thái: `CHECKPOINT_IMPLEMENTED_FUTURE_GOVERNANCE_REDESIGN_PLANNED`
...
Migration tương lai phải forward-only từ V28, dự kiến V29 hoặc số kế tiếp tại
...
### 2.3 Certificate -> skill -> task route là contract tương lai
...
Đây là future redesign slice, chưa được tuyên bố implemented trong checkpoint
`feature/practice-reduce-scope`. Nó cần implementation GO riêng và phải hoàn tất
trước khi mở rộng learner UI đa chứng chỉ ở Phase 13.
...
3. Backfill quan hệ từ dữ liệu V26/V27 hiện tại bằng forward migration; không
   sửa, squash hoặc checksum lại V25-V27.
...
- program/profile schema và seed:
  `src/main/resources/db/migration/V25__assessment_program_configuration.sql`;
- template roots và authoring identity:
  `src/main/resources/db/migration/V26__practice_authoring_contract.sql`;
- template versions, governance audit và material references:
  `src/main/resources/db/migration/V27__practice_phase12_governance.sql`;
- program/template/profile create/activate logic:
