...
class SpeakingPromptAuthoringControllerTest {
    ...
    @Test
    void excelStagingAdoptionIsIdFreeAcceptedAndNeverCallsTts() {
        Fixture fixture = fixture();

        ResponseEntity<?> response = fixture.controller.adoptExcelStaging(
                91L,
                "speaking-a",
        ...
        verify(fixture.uploadCoordinator)
                .adoptExcelStagingAndEnqueueStt(command.capture());
        assertThat(command.getValue().draftId()).isEqualTo(91L);
        assertThat(command.getValue().questionClientId())
                .isEqualTo("speaking-a");
        assertThat(command.getValue().actorId()).isEqualTo(81L);
        assertThat(command.getValue().expectedSourceRevision()).isEqualTo(7L);
        assertThat(command.getValue().expectedDraftVersion()).isEqualTo(3L);
        verify(fixture.authoring, never()).requestTts(any());
    }

    @Test
    void excelCoordinatorPreflightsThenVerifiesOutsideLockedBind() {
        ...
        SpeakingPromptOriginalAudioUploadCoordinator coordinator =
                new SpeakingPromptOriginalAudioUploadCoordinator(
                        authoring, assets);
        SpeakingPromptAuthoringService.SourceCommand command =
                new SpeakingPromptAuthoringService.SourceCommand(
                        91L, "speaking-a", 81L, 7L, 3L);
        byte[] bytes = "verified".getBytes(
                java.nio.charset.StandardCharsets.UTF_8);
        SpeakingPromptAssetService.VerifiedOriginalUpload verified =
                new SpeakingPromptAssetService.VerifiedOriginalUpload(
                        91L,
                        "speaking-a",
                        80L,
                        101L,
                        new SpeakingPromptAiContract.VerifiedAudio(
                                ...
                                "audio.mp3",
                                "audio/mpeg",
                                SpeakingPromptAiContract
                                        .exactBytesSha256(bytes),
                                1_000L));
        when(authoring.requireExcelStagingAdoptionAllowed(command))
                .thenReturn(80L);
        when(assets.verifyExcelStaging(91L, 80L, "speaking-a"))
                .thenReturn(verified);

        coordinator.adoptExcelStagingAndEnqueueStt(command);

        org.mockito.InOrder order = org.mockito.Mockito.inOrder(
                authoring, assets);
        order.verify(authoring)
                .requireExcelStagingAdoptionAllowed(command);
        order.verify(assets)
                .verifyExcelStaging(91L, 80L, "speaking-a");
        order.verify(authoring)
                .bindVerifiedExcelStaging(command, verified);
        verify(authoring, never()).requestTts(any());
    }
