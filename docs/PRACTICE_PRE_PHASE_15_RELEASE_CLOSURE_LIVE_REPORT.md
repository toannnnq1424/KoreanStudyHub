...
### 6.43 TOPIK 35 publisher/versioning rehearsal audit

Status: `PUBLISH_PREFLIGHT_BLOCKED / NO_PRIVATE_VERSION_STATE /
ZERO_TOPIK35_VERSION_DELTA`.

On `2026-08-04`, the existing publisher and immutable-version workflow was
audited and exercised against a newly migrated disposable MySQL 8.4 catalog,
`ksh_test_topik35_candidate_publish_audit_20260804c`. The test selected an
active `LECTURER` with effective `practice.publish` authority and used that
same database-issued user ID for the canonical draft, private local check-audio
asset and material reference. A different existing user remained unable to
bind, read or delete that asset.

The supported publisher cannot allocate a private or release-candidate version.
`PracticePublisherService` always persists the live set as `PUBLISHED`, copies
the draft scope, changes the draft to `PUBLISHED`, and then asks
`PracticePublishedVersionService` for a version whose status is also
`PUBLISHED`. The TOPIK 35 candidate scope is `GLOBAL`. The learner catalogue
selects exactly `status=PUBLISHED AND scope=GLOBAL`, while the learner service
and attempt-start boundary accept a set based on `PUBLISHED`. V25 permits only
`DRAFT`, `PUBLISHED` and `ARCHIVED` set states and has no release-candidate or
learner-visibility field. Calling this workflow successfully would therefore
make the candidate logically learner-visible; archiving afterward would leave
an unsafe visibility window and would not change the immutable version's
published identity.

The data preflight also remains intentionally red. All 50 Reading and 50
Listening questions declare the current typed explanation strategy, but none
has an approved editorial revision bound to its current question fingerprint.
`ObjectiveExplanationEditorialService` returned exactly 100
`OBJECTIVE_EXPLANATION_APPROVAL_MISSING` blockers. Two consecutive publisher
attempts by the authorized owner both failed at that approval gate, before the
first set mutation. The transaction/read-back invariants after each attempt
were identical:

| Marker | Result |
|---|---:|
| Flyway maximum / failed rows | `96 / 0` |
| canonical candidate | `1`, owner `2`, `DRAFT`, no published set |
| approved R/L explanation revisions | `0 / 100` |
| repeated publish attempts | `2`, both rejected before mutation |
| TOPIK 35 live sets | `0` |
| published-version count | `7 -> 7` (migration baseline only) |
| question-version count | `10 -> 10` (migration baseline only) |
| active private LOCAL check-audio asset/reference | `1 / 1`, owner `2` |
| provider execution audits / capability runs | `0 / 0` |

This means the current workflow can allocate immutable set/test/section/group/
question version IDs only as part of learner-visible publication; it cannot
satisfy the requested private rehearsal. No TOPIK 35 immutable ID was invented
or allocated. Repeat-publish and successful historical-version read/rollback
semantics were not exercised because reaching them would require bypassing the
two real gates above. The tested repeat-failure semantics are deterministic and
rollback-clean.

All prior candidate controls remained unchanged throughout the audit:
`publicationAllowed=false`; one continuous Listening program with start-once,
no seek/replay, no timestamp navigation/highlight and optional
`PENDING_OPTIONAL_POST_TEST_QA` timing; all Q51-Q54 answer contracts remain
`MANUAL_OR_EXPERIMENTAL_UNSCORED`; the check-audio asset is `PRIVATE`, `LOCAL`,
owner-scoped and referenced through `/practice/materials/{id}/content`; and no
AI/provider call was made.

Exact decisions/evidence required for a later successful rehearsal are:

1. editorial/data owners must approve 100 source-grounded typed R/L
   explanations bound to the current immutable question fingerprints. They
   must not be fabricated or silently generated by AI;
2. product must authorize a private staging lifecycle. The recommended default
   is a forward-only `RELEASE_CANDIDATE`/`learner_visible=false` state with an
   explicit release transaction, owner/reviewer authorization, catalogue and
   attempt-start denial before release, material-promotion semantics, immutable
   version read rules and deterministic repeat/rollback behavior; and
3. only after both controls exist should the publisher be exercised through
   successful publish/repeat/version-read/rollback. Directly invoking the
   snapshot service, inventing a DRAFT live graph, changing the candidate to
   `CLASS`, or publishing then archiving are not safe substitutes.

JDK `17.0.19` evidence: publisher/version/catalog/material and canonical
R/L/W seed regressions passed `90/90`; the fresh disposable persistence gate
passed `1/1`,
including 100-blocker preflight, two fail-closed publisher attempts, complete
data-plane count equality after each attempt, owner isolation, exact asset-byte
delivery, withdrawal/deletion/rebind regression and zero provider calls. The
container peaked at about `614 MiB` of its `640 MiB` limit and was then stopped
