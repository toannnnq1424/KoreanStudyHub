...
- browser QA không chạy trong baseline checkpoint, nên closure chưa từng đóng;
  ...
  `PHASE_12_POST_COMMIT_AUDIT = ACTION_REQUIRED` và
...
| 12 | `COMMITTED_BASELINE_POST_AUDIT_CONTINUATION_REQUIRED` | 12A-12E baseline đã có và 12F từng xanh 1293/1293, nhưng post-commit audit xác nhận P0 historical material access và program/template activation cùng UI/route/JDK 17/browser gaps | Cao cho baseline đã test; closure hiện NO-GO | Triển khai 12G theo continuation plan; R2/evaluator release debt giữ ở Phase 15; chưa mở Phase 13 |
...
### 12H - Future certificate, skill and task route redesign

Implementation: `FUTURE_SEPARATE_GO_REQUIRED`.

Quyết định scope ngày 2026-07-12:

- mỗi certificate/program chỉ có một scenario hoạt động tại một thời điểm;
  thay đổi cấu hình tạo immutable version mới, không tạo nhiều scenario cùng bật;
- mỗi skill được bật tự động có route luyện riêng. Bỏ delivery dropdown
  `Theo kỹ năng / Full test / Cả hai` khỏi policy UI;
- việc skill có nằm trong full test hay không thuộc active scenario composition,
  cùng thứ tự, timer và transition;
- mỗi skill có task routes động. TOPIK II Writing có thể seed Q51-Q54, nhưng
  KLAT/KLPT/OPIc/TOPIK Speaking và certificate khác tự định nghĩa task riêng;
- mỗi task route tham chiếu exact scoring/prompt/rubric profile versions và
  Admin/Head phải xem được approved system rules/runtime-support state;
- Admin/Head thao tác form có label, validation và diff; JSON chỉ được serialize
  nội bộ sau khi lưu. UI không được hiển thị raw HTML error response;
- lecturer editor chọn certificate trước và chỉ nhận active scenario/policy hợp
  lệ; learner library/progress giữ certificate context rõ ràng;
- migration schema tiếp theo phải forward-only sau V28, không sửa V28 hoặc
  rewrite old draft/version/attempt/result identity.

Target learner route map:
...
Các attempt/result routes canonical hiện có tiếp tục khóa set/test/version thực
tế. 12H phải hoàn tất trước learner multi-certificate expansion của Phase 13,
nhưng không được suy diễn là đã triển khai trong checkpoint
...
- continuation checkpoint đã có code/test tự động, nhưng browser closure còn mở
  và future 12H redesign chưa bắt đầu.
...
Current next action: preserve the implemented continuation checkpoint on
`feature/practice-reduce-scope`, then review the future 12H certificate/skill/
task redesign under a separate GO. Không merge, tuyên bố product GO hoặc mở
Phase 13 trước khi Phase 12 JDK 17/browser closure gate xanh.
