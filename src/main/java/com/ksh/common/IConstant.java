...
public interface IConstant {
    ...
    // ───────── Account activation (public, token-authorised) ─────────
    /** Public activation endpoint; {@code permitAll} in SecurityConfig. */
    String URL_ACTIVATE  = "/activate";
    String VIEW_ACTIVATE = "auth/activate";

    /**
     * Shown as a toast on the login page after a completed activation. Uses
     * {@code flashSuccess} rather than a bespoke key because login.html only
     * drains {@code flashSuccess} / {@code flashError} into {@code UlpToast}.
     */
    String MSG_ACTIVATION_COMPLETE =
            "Kích hoạt tài khoản thành công. Bạn có thể đăng nhập bằng mật khẩu vừa đặt.";

    // ───────── Admin roster import (admin-user-import-activation) ────
    String URL_ADMIN_USERS        = "/admin/users";
    String URL_ADMIN_USERS_IMPORT = URL_ADMIN_USERS + "/import";

    /** Sub-paths of {@code URL_ADMIN_USERS_IMPORT}, consumed by the modal script. */
    String PATH_IMPORT_UPLOAD   = "/upload";
    String PATH_IMPORT_CONFIRM  = "/confirm";
    String PATH_IMPORT_TEMPLATE = "/template";

    /** Admin-initiated activation re-send, appended to {@code /admin/users/{id}}. */
    String PATH_RESEND_ACTIVATION = "/resend-activation";

    String MSG_ACTIVATION_RESENT =
            "Đã gửi lại email kích hoạt";
    String MSG_ACTIVATION_RESEND_NOT_PENDING =
            "Tài khoản này đã kích hoạt, không cần gửi lại email";
    String MSG_ACTIVATION_RESEND_USER_MISSING =
            "Không tìm thấy tài khoản";
    String MSG_ROSTER_IMPORT_READ_FAILED =
            "Có lỗi không mong muốn khi đọc file.";
    String MSG_ROSTER_IMPORT_CONFIRM_FAILED =
            "Có lỗi không mong muốn khi import.";
