...
>
> Vị trí roadmap hiện tại: baseline Phase 13D đã xong; bounded
> `PHASE_13D_UX_CORRECTION` đang `IMPLEMENTED_PENDING_PHASE_VALIDATION`; Phase
> 13E vẫn chưa mở. Tài liệu này phục vụ nghiên cứu/design cho các bước sau, không mở Phase 13E, không
> coi Phase 13/14 đã kết thúc và không cho phép làm sớm gate trước Phase 15.
>
...
| Speaking Fluency/Pronunciation/Delivery | Phase 13D UX-03..05 đã truyền authoritative transcript confidence và khóa hai tiêu chí acoustic ở `NOT_SCORABLE`, nhưng evaluator vẫn không nhận audio | **NO-GO** cho điểm audio-grounded; guard hiện tại `IMPLEMENTED_PENDING_PHASE_VALIDATION` |
...
> transcript confidence, capability contract và semantics `NOT_SCORABLE` ở
> working tree, trạng thái `IMPLEMENTED_PENDING_PHASE_VALIDATION`. Chúng không
> còn là mô tả current source; các mục E-H và direct-audio/calibration/privacy
> vẫn là future debt theo P15-PRE-01/04/07/08/09.
...
Giữ sáu diagnostic scoring rows hiện tại nếu sản phẩm cần feedback từng blank:
...
Taxonomy nên có ba tầng, không tạo enum cho từng biểu hiện nhỏ:
...
| `DISCOURSE` | global organization, paragraphing, logical relation, cohesion/reference, transition use, redundancy |
| `MORPHOSYNTAX` | particle, ending/conjugation, tense/aspect, connective, modifier clause, valency, negation, passive/causative, sentence completeness, 호응 |
| `LEXICO_SEMANTIC` | word choice, collocation, idiomaticity, domain vocabulary, repetition, precision, Sino-Korean usage |
...
chỉ phải nhất quán với level/impact.
...
## 17. Lộ trình triển khai tương lai (chưa phải lệnh thực thi)

Các lát cắt dưới đây mô tả thứ tự phụ thuộc kỹ thuật. Chúng không thay đổi
`Current Required Next Action`: hiện tại phải hoàn tất static reconciliation,
đưa `PHASE_13D_UX_CORRECTION` tới `READY_FOR_PHASE_VALIDATION` và chạy đúng một
gate tổng hợp; Phase 13E chỉ bắt đầu sau GO riêng. Khi 13E-13H hoặc Phase 14 đã giải quyết đúng một mục, bước
pre-Phase-15 sau này chỉ kiểm chứng lại bằng evidence, không triển khai trùng.
...
2. `[IMPLEMENTED_PENDING_PHASE_VALIDATION]` Truyền authoritative
   `transcriptConfidence` qua Speaking request.
3. `[IMPLEMENTED_PENDING_PHASE_VALIDATION]` Speaking không audio: Fluency và
...
six-row envelope (bốn language `SCORED`, hai acoustic null `NOT_SCORABLE`). Tập
con này ngăn claim sai ngay bây giờ nhưng không thay cho registry Korean
construct, calibration và SME sign-off đầy đủ của `P15-PRE-14`.
...
Sau khi bounded `PHASE_13D_UX_CORRECTION` có consolidated green validation và
chỉ khi Phase 13E có GO riêng, KSH phải thay hai legacy detail parser bằng đúng
**ba màn/contract**:
...
file nào hiện đang sở hữu trách nhiệm và proof nào bắt buộc trước Phase 15.

Đến future pre-Phase-15 gate, cần re-scan vì Phase 13E-13H/14 có thể đã thay thế
`result-detail.html`, `rl-result-detail.html`, DTO hoặc service hiện tại. Re-scan
không được dùng để bỏ trách nhiệm; nó chỉ ánh xạ trách nhiệm sang file kế nhiệm.
...
`P15-PRE-14` trong inventory là gate thực thi cho section này; không thuộc bounded
Phase 13D correction hiện tại.
