...
/** Shared client for the official Korean Basic Dictionary Open API. */
@Component
public class KoreanDictionaryClient {

    private final KoreanDictionaryHttpClient httpClient;
    ...
    public KoreanDictionaryClient(
            KoreanDictionaryHttpClient httpClient,
            @Value("${app.dictionary.api-key:}") String apiKey,
            @Value("${app.dictionary.base-url:https://krdict.korean.go.kr/api/search}") String baseUrl,
    ...
    KoreanDictionaryClient(KoreanDictionaryHttpClient httpClient, String apiKey, String baseUrl) {
        this.httpClient = httpClient;
    ...
    public Optional<DictionaryEntry> lookupVietnamese(String koreanWord) {
        ...
        String url = UriComponentsBuilder.fromUriString(effectiveBaseUrl())
                ...
                .encode()
                .toUriString();
        return parse(httpClient.get(url, MediaType.APPLICATION_XML), koreanWord);
    }
    ...
    Optional<DictionaryEntry> parse(String xml, String requestedWord) {
        try {
            ...
            if (meanings.isEmpty()) return Optional.empty();
            ...
            if (dictionaryUrl == null || dictionaryUrl.isBlank()) {
                dictionaryUrl = "https://krdict.korean.go.kr/vie/dicSearch/SearchView?ParaWordNo=" + targetCode;
            }
    ...
    private static String translationMeaning(Element translation) {
        ...
        if (translatedWord == null) return definition;
        if (definition == null || definition.equalsIgnoreCase(translatedWord)) return translatedWord;
