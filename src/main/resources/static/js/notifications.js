 ...
 *
 * Dependencies: app.js (dropdown toggle + KshToast) loaded before this file.
 */
(function () {
  'use strict';
  ...
  function drainFlash() {
    ...
    if (info && info !== 'null' && info.trim()) {
      window.KshToast && window.KshToast.info
        ? window.KshToast.info(info.trim())
        : (window.KshToast && window.KshToast.success(info.trim()));
    }
  }
