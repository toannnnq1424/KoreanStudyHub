...
- UI kho gọi `PracticeMaterialLibraryPageController.page` qua `GET /practice/manage/materials`. `PracticeMaterialLibraryService.catalog` chỉ trả asset của owner và reference thuộc bộ đề owner quản lý; V114 đã retire shared collaborator catalog.
...
- Preview/audio/link dùng `GET /practice/materials/{assetId}/content` → `PracticeMaterialController.content` → `PracticeMaterialAccessService.load`. Service cho owner; learner có exact attempt; người dùng authenticated đọc được reference của current `PUBLISHED + GLOBAL` set hoặc ACTIVE enrollment của class. Biết asset id không đủ quyền.
